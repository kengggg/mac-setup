...
- **Idempotency is a hard invariant.** Every component must be safely re-runnable: `brew_install` skips installed packages, `clone_if_absent`, `ensure_local_block` refreshes paired installer-owned markers in `~/.zshrc.local`, preserves handwritten text outside markers, and backs up changes. Duplicate/incomplete markers fail unchanged; only the exact legacy unmarked nvm block is auto-adopted. Keep this property when editing components.
